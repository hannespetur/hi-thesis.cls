...
\renewcommand\nomgroup[1]{
    \ifthenelse{\equal{#1}{F}}{\item[\textbf{Rice's Framework}]}{
        \ifthenelse{\equal{#1}{J}}{\item[\textbf{Job-Shop Scheduling}]}{ 
...
\nomenclature[f05]{$Y$}{Mapping for algorithm and feature space onto 
performance space, i.e., $y=Y(a,\vec{\phi}(\vec{x}))\in\mathcal{Y}$ where 
$Y:\mathcal{A}\times\mathcal{F}\mapsto\mathcal{Y}$}
...
\nomenclature[j12]{$\vec{\chi}$}{sequence of dispatches $J_j$ to create 
(partial) schedule/solution} 
\nomenclature[j13]{$\mathcal{U}(u_1,u_2)$}{uniform distribution from the 
interval $I=[u_1,u_2]\subset\mathbb{R}$}
...
% ACRONYMS
\nomenclature[z01]{JSP}{Job Shop scheduling problem}
\nomenclature[z02]{FSP}{Flow Shop scheduling problem}
\nomenclature[z03]{DR}{Dispatching rule}
\nomenclature[z04]{SDR}{Single priority dispatching rule}
